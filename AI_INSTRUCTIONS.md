...
## Project Overview
Pramnos Framework is a modern, independent PHP framework requiring **PHP 8.1+** (developed and tested on PHP 8.5). It is
...
2. **PHP 8.1+.** All production code in `src/Pramnos/*` must run on PHP 8.1+
   (dev/CI uses PHP 8.5). Use typed properties, match expressions, named arguments,
   and native attributes freely.
...
| `app` | custom PHP 8.5 + Apache | Test runner / web server |
