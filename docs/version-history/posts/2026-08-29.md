...
The older tables stay where they are. `deferredwrites` would fit `pramnos` on the same
reasoning, but it has been in `public` since 12 August and moving a table that exists in a
deployment is a data migration, not a rename.
