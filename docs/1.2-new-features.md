...
- **`Pramnos\Queue\Drivers\RedisQueueDriver`** — the shipped driver. A Redis sorted
  set scored by run-at time plus a companion payload hash; atomic claim-by-`ZREM`.
  Self-connects from config **or** reuses an injected `\Redis` connection factory
  (the same pattern as the broadcasting `RedisDriver`), and prefixes its own keys
  (no reliance on `OPT_PREFIX`) so a migrating app keeps byte-identical keys.
