...
When `auth()` is called with an empty addon registry (no `onAuth` handlers), the method now logs a warning to `auth.log` in addition to returning `false`.
...
This replaces the previous silent failure, which made misconfigured apps extremely difficult to diagnose.
