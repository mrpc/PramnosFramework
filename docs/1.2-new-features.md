...
Answering `y` (or passing `--rest-api=y`) triggers two additional scaffold actions:

### Directory + routes file

```
src/Api/Controllers/       ← namespace for API controllers
src/Api/routes.php         ← Router::group() example with /v1 prefix
```

`src/Api/routes.php` content:
...
$router->group(
    ...
    function (\Pramnos\Routing\Router $r): void {
        // $r->get('/hello', [\MyApp\Api\Controllers\HelloController::class, 'index']);
    }
);
```

The `{{ namespace }}` placeholder is substituted with the actual application namespace at scaffold time.
...
- `tests/Unit/Console/InitCommandUnitTest.php` — 4 new tests:
  - `testRestApiOptionScaffoldsApiDirectoryAndRoutesFile` — `src/Api/Controllers/` + `src/Api/routes.php` created when `--rest-api=y`
  - `testRestApiRoutesFileContainsRouterGroupAndNamespaceComment` — routes.php contains `$router->group()`, `/v1` prefix, namespace substituted
  - `testRestApiOptionAddsApiSectionToAppPhp` — `app.php` contains `'api'` section with `prefix`, `cors_origins`, `version`
  - `testNoRestApiOptionSkipsApiScaffolding` — no `src/Api/` directory and no `'api'` key in `app.php` when option is absent
