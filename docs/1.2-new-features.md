...
|------|----------|---------|
| `000031_create_authserver_user_deyas_table.php` | 45 | `authserver.user_deyas` — org membership |
...
**`authserver.user_deyas`**: Composite PK `(userid, deyaid)`. Tracks which users belong to which organisations. A user must be a member before being assigned an organisation-scoped role (deyaid IS NOT NULL). Columns: `userid BIGINT`, `deyaid INT`, `granted_by BIGINT` (nullable), `granted_at TIMESTAMP`, `expires_at TIMESTAMP` (nullable), `is_active BOOL`.

**`authserver.permission_templates`**: Blueprint for a single permission grant. `templateid SERIAL PK`, `template_name VARCHAR(100)` (unique), `template_type` (role_template/deya_template/user_template), `object_type VARCHAR(50)`, `object_id_pattern VARCHAR(100)` (supports `{deyaid}` placeholder), `action`, `grant_type`, `priority`, `is_active`, `created_at`, `created_by`.
...
- `object_id` changed from `BIGINT` to `VARCHAR(100)` — required to support wildcards (`*`) and non-integer IDs (e.g., `deya_{id}` patterns from templates).
...
| `check_user_deya_membership()` | trigger fn | Blocks role assignment if user is not an org member |
| `apply_permission_template()` | `(templateid, subject_type, subject_id, context_deyaid, granted_by) → INTEGER` | Creates one permission row from a template |
| `apply_role_template()` | `(role_templateid, role_name, deyaid, created_by) → INTEGER` | Creates a role and applies all its permission templates |
| `log_audit_event()` | `(action_type, performed_by, ...) → BIGINT` | Inserts an `audit_log` row |
...
| `get_user_effective_permissions()` | `(userid, deyaid) → TABLE` | Returns all effective permissions for a user (direct + role-based) |
...
$perms = $db->query(
    "SELECT * FROM authserver.get_user_effective_permissions(:userid, :deyaid)",
    [':userid' => $userId, ':deyaid' => $orgId]
);
while ($perms->fetch()) {
...
$db->query(
    "SELECT authserver.apply_role_template(:templateid, :rolename, :deyaid, :createdby)",
    [':templateid' => 1, ':rolename' => 'org_admin', ':deyaid' => 42, ':createdby' => $adminUserId]
);
```
...
MySQL (`FrameworkMigrationsMySQLTest`): 5 new tests covering user_deyas, permission_templates, role_templates, permission_inheritance table creation, and effective_permissions VIEW including deny-takes-priority assertion.
...
- ✅ application_stats hypertable
- ✅ user_app_authorizations table
- ✅ Missing foreign keys schema import

**In Progress:**
- ⏳ Analytics and monitoring views (18 total)
- ⏳ Integration tests across MySQL, PostgreSQL, TimescaleDB
