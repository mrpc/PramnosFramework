...
| `cache` | `?action=cache` | Cache adapter type; flush button (POST) |
| `users` | `?action=users` | Active sessions; login lockouts |
| `performance` | `?action=performance` | Slowest endpoints — time range filter |
...
`tests/Unit/Webhook/WebhookHandlerTest.php` — 17 tests covering:
