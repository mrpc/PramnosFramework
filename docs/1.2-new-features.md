...
## 26. Phase 2: CLI UX Improvements — PramnosStyle

**Class:** `Pramnos\Console\Style\PramnosStyle`

Extends `Symfony\Component\Console\Style\SymfonyStyle` with framework-specific console helpers. All standard SymfonyStyle methods (`success()`, `error()`, `warning()`, `info()`, `table()`, `progressStart()`, `progressAdvance()`, `progressFinish()`, etc.) are inherited unchanged.

### Framework-specific additions

| Method | Description |
|---|---|
| `frameworkTitle(string $title)` | Prominent cyan title banner — use once at the top of a multi-step command. |
| `step(int $current, int $total, string $label)` | Wizard step header: `Step 2/6: Framework features` |
| `summaryTable(array<string,string> $pairs)` | Two-column label/value table with no header — for configuration summaries. |
| `statusTag(string $status): string` *(static)* | Wraps `Ran`→green, `Failed`→red, `Pending`→yellow, unknown→unchanged. |
| `hint(string $message)` | Dimmed secondary guidance text. |
| `check(string $message)` | `✓` green check mark + message (individual step confirmations). |
| `cross(string $message)` | `✗` red cross mark + message (individual step failures). |
...
protected function execute(InputInterface $input, OutputInterface $output): int
{
    ...
    $io->frameworkTitle('Project Initialisation');

    $io->step(1, 6, 'Project metadata');
    // ... collect input ...

    $io->step(6, 6, 'Docker startup');
    $io->progressStart(3);
    // ... long operation ...
    $io->progressAdvance();
    $io->progressFinish();
...
- `PramnosStyle` is a purely additive class — it does not modify any existing command.
- Commands can continue using `OutputInterface` / `InputInterface` directly; adoption is opt-in.
- `statusTag()` is a static helper and can be called without an instance.

### Tests

`tests/Unit/Console/Style/PramnosStyleTest.php` — 14 unit tests covering: `frameworkTitle()`, `step()` format, single-step edge case, `summaryTable()` content, empty summaryTable, `statusTag()` all four variants, `hint()`, `check()`, `cross()`, and inherited `success()`/`error()` smoke tests.
