...
Dashboard di gestione account utente. Tutte le azioni richiedono login (`addAuthAction`).

| Azione | HTTP | Descrizione |
|--------|------|-------------|
| `display` | GET | Panoramica: app autorizzate + attività recente |
| `applications` | GET | Lista applicazioni con token attivi |
| `revokeapplication` | POST | Revoca accesso per un'app (AJAX JSON o redirect) |
| `exportdata` | GET | Download JSON dati personali (GDPR Art. 20) |
| `deleteaccount` | GET/POST | Cancellazione account (GDPR Art. 17) |
| `privacy` | GET/POST | Impostazioni consenso |
| `security` | GET | Panoramica sicurezza + stato 2FA |
| `changepassword` | GET/POST | Cambio password (policy: ≥8 chars, ≥1 cifra, ≥1 simbolo) |

`revokeapplication` imposta `usertokens.status = 3` (revocato, conservato per audit) e cancella `oauth2_user_consents`. Supporta AJAX (`X-Requested-With: XMLHttpRequest`) e form classico.

`deleteaccount` cancella righe da `usertokens`, `oauth2_user_consents`, `user_activity_log`, `user_privacy_settings`, `user_twofactor`, `twofactor_setup`, e infine da `users`.

`changepassword` verifica la password corrente (bcrypt o SHA-256+salt legacy), applica la policy, salva con `password_hash(..., PASSWORD_BCRYPT)`.
