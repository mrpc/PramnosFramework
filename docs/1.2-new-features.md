...
- [26. Phase 2: CLI UX — CommandBase (SchemaImport από the reference application)](#26-phase-2-cli-ux-commandbase-schema import-the reference application)
...
- [the reference application Schema SchemaImport — Phase 2](#the reference application-schema-schema import-phase-2)
...
Two capabilities added to align with the SchemaImport Spec, usable in `ifCapable()` calls:
...
### Migration-support helpers (added for SchemaImport migrations)

The following public methods were added to expose the migration API cleanly to `Migration` subclasses and to align with the SchemaImport Spec's calling convention.
...
Alias for `alterTable()`. Matches the Laravel `Schema::table()` calling convention used in SchemaImport migrations.
...
Alias for `->constraintName()`. Matches the SchemaImport Spec's `->name()` chaining style.
...
been wired up yet (expected during Phase 4 before the feature implementations land).
...
- All currently registered built-in features (`core`, `auth`, `authserver`,
  ...
  will be instantiated for them until the feature implementations are implemented.
...
All schemas match the reference application (UW) production database. Schema comments in the
...
## 26. Phase 2: CLI UX — CommandBase (SchemaImport από the reference application)

**Class:** `Pramnos\Console\CommandBase`

Abstract base class for all lock-guarded and interactive console commands. SchemaImported from `App\ConsoleCommands\CommandBase`. the reference application commands can now extend `Pramnos\Console\CommandBase` directly and inherit all infrastructure via PHP's normal inheritance chain.
...
the reference application's `CommandBase` can then extend `\Pramnos\Console\CommandBase` and override only what differs (e.g. `getOrchestratorCommandName()`), so application code remains unchanged.
...
- `getOrchestratorCommandName()` returns `'daemons:start'` by default — matches the reference application's current orchestrator command name.
...
**Problem:** the reference application contained a complete background job queue (QueueManager, Worker, task handlers, CLI commands) but it was tightly coupled to the reference application-specific models, controller names, namespace paths, and dashboard titles, making it impossible to reuse in other applications.

**Solution:** Full schema import to `Pramnos\Queue` and `Pramnos\Console\Commands` with configurable hooks at every the reference application-specific point. Applications subclass and override only what differs; all infrastructure is inherited.
...
### Migration from the reference application

```php
// Before
use the reference application\Services\Queue\QueueManager;
use the reference application\Services\Queue\Worker;

// After
use Pramnos\Queue\QueueManager;
use Pramnos\Queue\Worker;
// Override hooks to restore the reference application-specific behaviour.
```
...
**Problem:** the reference application contained a monolithic `DaemonOrchestrator` that managed background daemons (queue workers, Kafka consumers). All the process-supervision infrastructure was tightly coupled to the reference application-specific settings and entry-points, making it impossible to reuse in other applications.
...
- `getOrchestratorCommandName()` (inherited from `CommandBase`) returns `'daemons:start'` by default — matches the reference application's current command name.
...
**SchemaImport from:** `Authserver\Models\Loginlockout`
...
**SchemaImport from:** `Authserver\Helpers\TOTPHelper` + `Authserver\Services\TwoFactorAuthService`
...
**SchemaImport from:** `Authserver\Helpers\Scopes` + `Authserver\Helpers\OAuthPolicyHelper`
...
**Problem:** The existing authserver schema had basic roles, permissions, user_roles, and audit_log tables (migrations 000020–000024) but lacked the complete RBAC infrastructure needed for the reference application-style multi-organisation permission system: organisation membership, permission/role templates, object hierarchy, the effective_permissions view, and the PL/pgSQL helper functions.
...
**SchemaImport from:** `Authserver\Services\WebhookService`
...
**Problem:** the reference application needed descriptive lists of OAuth2 authentication methods, grant types, and webhook event types for developer-facing UIs. These were maintained in the reference application's `PermissionHelper` class, creating a drift risk.
...
**Problem:** When issuing tokens, callers needed to merge the requested scopes with the server's default scopes. This logic was duplicated in the reference application.
...
**Problem:** Geographic coordinate validation and IP/CIDR validation were implemented in the reference application's `HelperFunctions` but are general-purpose utilities with no application-specific logic.
...
## the reference application Schema SchemaImport — Phase 2

**Status:** In Progress (v1.2-dev)

The Pramnos Framework v1.2 incorporates advanced schema elements from the reference application reference application to provide production-grade monitoring, security, and compliance features out of the box.
...
- ✅ Missing foreign keys schema import (core/000050)
...
The `_jsonactions` mechanism (the reference application BC — appends edit/delete links to rows) is preserved. `Datasource::getList()` is no longer called from `_getJsonList()`.
...
**Problem:** After session 113, `auth` was wired but `authserver` (OAuth2 server) was left unwired — no controller wrapper existed to route `/oauth/authorize`, `/oauth/token` etc. Similarly, every new app should expose the log viewer at `/logs` (following the reference application pattern) but no `Logs` controller was scaffolded.
...
Scaffolded for every new application (no feature flag needed). Follows the reference application pattern: thin wrapper extending `\Pramnos\Application\Controllers\LogController`. Developers override `$whitelist` and `$blacklist` to control visible log files.
...
- **Security** — `securitySalt` (write-only field), `loginlockoutwindowseconds`, `loginlockoutsteps` (dynamic card builder, same format as the reference application)
