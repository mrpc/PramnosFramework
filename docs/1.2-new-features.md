...
├── auth/
│   ├── 2020_01_01_000010_create_roles_table.php
│   ├── 2020_01_01_000011_create_users_table.php
│   ├── 2020_01_01_000012_create_user_roles_table.php
│   └── 2020_01_01_000013_create_password_resets_table.php
├── authserver/
│   ├── 2020_01_01_000020_create_oauth_clients_table.php
│   ├── 2020_01_01_000021_create_oauth_scopes_table.php
│   ├── 2020_01_01_000022_create_oauth_authorization_codes_table.php
│   └── 2020_01_01_000023_create_oauth_tokens_table.php
├── messaging/
│   ├── 2020_01_01_000030_create_message_threads_table.php
│   ├── 2020_01_01_000031_create_messages_table.php
│   └── 2020_01_01_000032_create_message_recipients_table.php
└── queue/
    ├── 2020_01_01_000040_create_jobs_table.php
    ├── 2020_01_01_000041_create_failed_jobs_table.php
    └── 2020_01_01_000042_create_job_batches_table.php
```
...
**`framework_policies`** — Stores scheduled database maintenance policies (retention, aggregate-refresh, compression, cache-rebuild). Used by `PolicyEngine`.

| Column | Type | Notes |
|---|---|---|
| `policyid` | `SERIAL` / `AUTO_INCREMENT` | PK |
| `policy_type` | `VARCHAR(50)` | `retention`, `aggregate_refresh`, `compression`, `cache_rebuild` |
...
**`user_roles`** — Many-to-many user↔role pivot.

| Column | Type |
|---|---|
| `id` | PK auto-increment |
| `userid` | FK → `users.userid` CASCADE |
| `roleid` | FK → `roles.roleid` CASCADE |
| `assigned_at` | `TIMESTAMP` useCurrent |
| — | UNIQUE(`userid`, `roleid`) |

**`password_resets`** — Short-lived password reset tokens.
...
**`oauth_clients`** — Registered OAuth 2.0 client applications.
...
| `grant_types` | `VARCHAR(255)` nullable |
| `scopes` | `TEXT` nullable |
| `is_confidential` | `BOOLEAN` default `true` |
| `created_at` | `TIMESTAMP` useCurrent |

**`oauth_scopes`** — Defined permission scopes.

| Column | Type |
|---|---|
| `id` | PK auto-increment |
| `scope` | `VARCHAR(100)` unique |
| `description` | `TEXT` nullable |
...
| `threadid` | FK → `message_threads.threadid` CASCADE |
| `sender_id` | `INT` unsigned nullable |
| `body` | `TEXT` |
| `is_read` | `BOOLEAN` default `false` |
| `created_at` | `TIMESTAMP` useCurrent |

**`message_recipients`** — Per-user read state for each message.

| Column | Type |
|---|---|
| `id` | PK auto-increment |
| `messageid` | FK → `messages.messageid` CASCADE |
| `user_id` | `INT` unsigned |
| `is_read` | `BOOLEAN` default `false` |
| `read_at` | `TIMESTAMP` nullable |
| — | UNIQUE(`messageid`, `user_id`) |
...
**`failed_jobs`** — Jobs that exhausted retry attempts.
...
**`job_batches`** — State tracking for batch job groups.
...
| `pending_jobs` | `INT` unsigned default `0` |
| `failed_jobs` | `INT` unsigned default `0` |
| `failed_job_ids` | `TEXT` |
| `options` | `TEXT` nullable |
| `cancelled_at` | `INT` unsigned nullable |
| `created_at` | `INT` unsigned |
| `finished_at` | `INT` unsigned nullable |
