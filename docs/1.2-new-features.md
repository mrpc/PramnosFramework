...
## Phase 23 (partial) — Framework Admin CRUD Controllers
...
`pramnos init app` now always scaffolds:
- `src/Controllers/Users.php` → thin wrapper extending `UsersController`
- `src/Controllers/Settings.php` → thin wrapper extending `SettingsController`

Both are always created (not feature-gated).
