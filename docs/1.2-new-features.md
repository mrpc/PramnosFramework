...
| `database()` | HTML detail page: DB version/size/connections, active process list, table sizes, TimescaleDB data |
| `cache()` | HTML detail page: cache adapter, namespace list, item browser (key/size/TTL/type) |

`database()` collects data via three private helpers:
- `collectProcessList()` — MySQL: `SHOW PROCESSLIST`; PostgreSQL: `pg_stat_activity` with state, wait event, duration
- `collectTableSizes()` — MySQL: `information_schema.tables`; PostgreSQL: `pg_total_relation_size` per table
- `collectTimescaleData()` — PostgreSQL + TimescaleDB only: hypertables, continuous aggregates, scheduled jobs; gracefully returns empty arrays if the extension is absent

`cache()` calls `Cache::getStats()`, `getCategories()`, and `getAllItems()` per category (limit 50 items).

All six actions are auth-protected (`addAuthAction`). `requiredUserType = 80` (manager level).
