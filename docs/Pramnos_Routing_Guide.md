...
| `jobposts/view/479` | `jobposts` | `view` | `479` | **`jobposts => view`** |
| `a/b/c/d` | `a` | `b` | — | `c => d` |
...
### Two things in that table are anomalies

Recorded rather than fixed, because both decide **which page is served** for URLs already in
use, so changing them is a routing change for every installation and belongs in a deliberate
release.

**Leftover path segments become `$_GET` keys.** `module/action/x` sets `_option = x` *and*
`$_GET['module'] = 'action'`: parts 0 and 1 are not removed before the key/value pairing loop
walks what is left. On `/jobposts/view/479`, code reading `$_GET['jobposts']` gets `'view'`.

**A slash inside the query string changes the decomposition.** `$slashes` is recounted *after*
the query string is appended to the path, so an ordinary `?return=/account/settings` makes the
same path take a different branch:

```
r=jobposts/view/479                                → $_GET: _option=479, jobposts=view
r=jobposts/view/479  ?return=/account/settings     → $_GET: _option=479, 479=null
```

One slash is enough. `_option` survives in this shape, which is why it goes unnoticed — it is
the leftover keys that move, and code reading one of them by name reads something else.
