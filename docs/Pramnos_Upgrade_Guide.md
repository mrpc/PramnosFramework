...
| **`theme:build` output** | `ThemeTokens::parse()` strips CSS comments before reading `app/themes/theme.css`, so a `/* … */` inside a `@plugin` block no longer becomes a key in `www/assets/theme-tokens.json` (and no longer swallows the declaration after it). | Nothing at runtime — the keys that disappear were never valid tokens. **If your palette has comments, run `pramnos theme:build` once after upgrading**: the regenerated outputs differ, so `theme:build --check` fails in CI until they are committed. |
