...
| **Webhook endpoints on a private network** | `/Webhook/register` refuses an `endpoint_url` whose host resolves to loopback, a private or link-local range; `WebhookService` delivers through `Http\Client::forUserSuppliedUrl()`, which refuses the same addresses and `http://` at delivery time and does not follow redirects. `lastError` for a transport failure reads `Delivery refused or failed: …` instead of `cURL error: …`. | **An installation whose relying parties receive webhooks on an internal address stops delivering to them** — the events are recorded as failed, not lost silently: check `GET /Webhook/stats` or `authserver.webhook_events` for `failed` after upgrading. Give such a receiver a public address. A monitor that matched `cURL error:` in `last_error` should match `Delivery refused or failed:`. |
