...
class SchemaBuilderUnitTest extends TestCase
{
    ...
    // =========================================================================
    // compressChunk() / decompressChunk() — TimescaleDB
    // =========================================================================

    /**
     * Neither chunk operation does anything on a backend without TimescaleDB.
     *
     * They are called from the deferred-write drain, which runs on every
     * backend — on MySQL there are no chunks and nothing to do, and raising
     * there would make the drain backend-specific.
     */
    public function testChunkCompressionIsANoOpWithoutTimescaleDB(): void
    {
        // Arrange
        $sb = $this->makeSB('mysql');

        // Act & Assert
        $this->assertFalse($sb->compressChunk('_timescaledb_internal', '_hyper_1_1_chunk'));
        $this->assertFalse($sb->decompressChunk('_timescaledb_internal', '_hyper_1_1_chunk'));
    }

    /**
     * Both operations quote the chunk through PostgreSQL's own format('%I.%I').
     *
     * Internal chunk names are generated by TimescaleDB and live in a schema
     * whose name starts with an underscore; pasting them into SQL unquoted is
     * how this breaks on the one installation whose names need quoting. Asking
     * PostgreSQL to quote its own identifiers cannot get that wrong.
     */
    public function testChunkCompressionQuotesTheChunkIdentifiers(): void
    {
        // Arrange — prepareQuery behaves as the real one does: %s becomes a
        // quoted literal and %% collapses to a single %, via vsprintf.
        $db = $this->getMockBuilder(Database::class)
            ->disableOriginalConstructor()
        ...
        $db->method('prepareQuery')->willReturnCallback(
            static fn(string $format, ...$args): string => vsprintf(
                str_replace('%s', "'%s'", $format),
                $args
            )
        );
        $sb = new SchemaBuilder($db);
        ...
        // Act
        $sb->decompressChunk('_timescaledb_internal', '_hyper_1_1_chunk');
        $sb->compressChunk('_timescaledb_internal', '_hyper_1_1_chunk');

        // Assert
        $this->assertStringContainsString('decompress_chunk(', $sql[0]);
        $this->assertStringContainsString("format('%I.%I'", $sql[0]);
        $this->assertStringContainsString("'_hyper_1_1_chunk'", $sql[0]);
        $this->assertStringContainsString('compress_chunk(', $sql[1]);
    }
