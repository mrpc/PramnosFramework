...
class TwoFactorAuthServiceMySQLTest extends TestCase
{
    ...
    protected static function createTables(Database $db): void
    {
        ...
        $targets = [
            ...
            // Widens the seed columns to fit an encrypted value. Sorted by
            // priority above, so it runs after the tables it alters.
    ...
    public function testTotpSeedIsEncryptedInBothTables(): void
    {
        ...
        try {
            ...
            $setupRow = $this->db->query(
                'SELECT temp_secret FROM authserver_twofactor_setup WHERE userid = 90'
            );
            ...
            $userRow = $this->db->query(
                'SELECT secret FROM authserver_user_twofactor WHERE userid = 90'
            );
    ...
    public function testPlaintextSeedStillVerifiesAfterEncryptionIsEnabled(): void
    {
        ...
        $now    = time();
        $this->db->query(
            "INSERT INTO authserver_user_twofactor (userid, enabled, secret, created_at, updated_at)
