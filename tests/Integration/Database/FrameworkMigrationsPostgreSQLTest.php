...
class FrameworkMigrationsPostgreSQLTest extends TestCase
{
    ...
    /**
     * users table must have the full the reference application schema: BIGINT userid PK,
     ...
     * 'deyaid' for the reference application), the roles table must be created with that column
     ...
     * are overridden (the reference application pattern: table='user_deyas', column='deyaid'), the
    ...
    {
        try {
            // Arrange — configure the reference application-style naming before running the migration
     ...
     * event model that matches reference production (with organization_context
     ...
     * a unique constraint on (appid, auth_method), and the reference application-aligned
    ...
    public function testAuthserverOauth2ClientAuthMethodsCreatesTableInApplicationsSchemaOnPostgres(): void
    {
        ...
        // Assert — is_enabled column exists (renamed from is_active to match the reference application)
        $this->assertTrue(
     ...
     * the reference application-aligned column schema (webhook_id, endpoint_url, webhook_type,
     ...
     * authserver schema; index names match the reference application (uniq_loginlockouts_lookup,
    ...
    public function testUserPrivacySettingsCreatedInAuthserverSchemaOnPostgreSQL(): void
    {
        ...
        // Assert — the reference application-aligned column names exist; old analytics_consent must NOT exist
        $this->assertTrue(
            $this->columnExists('user_privacy_settings', 'share_usage_analytics', 'authserver'),
            'share_usage_analytics column must exist (the reference application column name)'
        );
        $this->assertTrue(
            $this->columnExists('user_privacy_settings', 'marketing_emails', 'authserver'),
            'marketing_emails column must exist (the reference application column name)'
        );
        $this->assertFalse(
            $this->columnExists('user_privacy_settings', 'data_processing', 'authserver'),
            'data_processing must not exist (not in the reference application schema)'
        );
        $this->assertFalse(
    ...
    protected function loadMigration(string $feature, string $class): \Pramnos\Database\Migration
    {
        ...
        foreach (glob($this->migrationsBase . '/*', GLOB_ONLYDIR) ?: [] as $other) {
            foreach (MigrationLoader::loadFromDirectory($other, $this->app) as $m) {
                if ((new \ReflectionClass($m))->getShortName() === $class) {
                    return $m;
                }
            }
        ...
        $this->fail("Migration class '{$class}' not found in feature '{$feature}' nor anywhere else under the framework migrations");
    }
