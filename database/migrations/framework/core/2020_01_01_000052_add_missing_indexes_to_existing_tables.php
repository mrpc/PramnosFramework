 ...
 *
 * Adds indexes present in the reference production schema that were not
...
class AddMissingIndexesToExistingTables extends Migration
{
    ...
    public function up(): void
    {
        ...
        // ===== users =====
        // idx_users_photo — present in the reference application, missing from framework migration.
        ...
        if ($schema->hasTable('tokenactions')) {
            // Simple single-column indexes from original the reference application schema.
            $this->addIndex('tokenactions', 'idx_tokenactions_tokenid', ['tokenid']);
            $this->addIndex('tokenactions', 'idx_tokenactions_urlid', ['urlid']);

            // return_status and execution_time_ms added in a later the reference application migration.
            ...
            // Composite (action_time, method) — mirrors the reference application idx_tokenactions_time_method.
