...
class CreateJwtReplayPreventionTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        $t      = $schema->quoteTable('authserver.jwt_replay_prevention');

        if ($schema->hasTable('authserver.jwt_replay_prevention')) {
            return;
        }
        ...
        if ($caps->isPostgreSQL()) {
            ...
            $db->query("CREATE INDEX IF NOT EXISTS idx_jrp_expires ON {$t} (expires_at)");
        } else {
            $db->query("CREATE TABLE IF NOT EXISTS {$t} (
    ...
    public function down(): void
    {
        ...
        $db->schema()->dropTableIfExists('authserver.jwt_replay_prevention');
    }
}
