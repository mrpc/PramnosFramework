# PramnosFramework vs the reference application Migration Audit
...
## Missing Migrations (the reference application Schema Elements)
...
**Missing FKs (from the reference application):**
...
## Missing Views (the reference application Schema)
...
## Missing Triggers/Functions (the reference application Schema)
...
1. **jwt_replay_prevention table**: Currently in public schema (correct), but migrations had it in authserver
   - Should stay in public schema (matches the reference application)
   
2. **slow_api_calls view**: Currently in authserver in framework
   - Should be in applications schema (matches the reference application)
