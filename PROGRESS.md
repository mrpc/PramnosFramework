...
### ✅ the reference application integration suite: 5208 / 5208 tests pass
...
## 🏁 Session 144 — the reference dev database BC validation: 3 regression fixes (2026-05-26)
...
### ✅ the reference dev database BC αποτελέσματα
...
- **`tests/Characterization/Database/SchemaMigrationsCharacterizationTest.php`**
...
- **`tests/Characterization/Database/SchemaMigrationsCharacterizationTest.php`** — 3 nuovi test: sessions indexes, tokenactions basic indexes, conditional column indexes.
...
- `[x]` Συγχρονισμός indexes με the reference application schema (sessions, users, usertokens, tokenactions)
...
### ✅ Extended DashboardController with full feature parity to the reference application (commit 7b876bc)
...
- Lockout steps dynamic card builder ported from the reference application (JS in all 3 themes)
...
**ROADMAP cleanup:** marked all the reference application Schema SchemaImport Tasks as `[x]` — migrations 000020–000048 cover all authserver/applications/public schema items.
...
- **Πάντα → `src/Controllers/Logs.php`**: Ακολουθεί το the reference application pattern (`class Logs extends LogController`). Scaffolds σε κάθε νέα εφαρμογή. URL: `/logs`. Developers override `$whitelist`/`$blacklist` για control των log files.
...
- **`www/index.php`:** Αντικατάσταση `Application::getInstance()` με `new \{Namespace}\Application()` (direct instantiation, όπως στο the reference application). Σωστή χρήση `exec()` χωρίς `echo`.
...
**Εκκρεμεί:** Client-side adapter unit tests (mock fetch), the reference application migration Phase 8.
...
## 🏁 Session 103 — the reference application DB sync: continued schema alignment (2026-05-22)
...
## 🏁 Session 102 — the reference application DB sync: boolean success + authserver views rewrite (2026-05-21)
...
### ✅ authserver views (000046) — full rewrite matching the reference application

All 7 monitoring views replaced with exact the reference application logic:
...
- `9e914e8` fix(migrations): rewrite authserver views to match reference production

---

## 🏁 Session 101 — the reference application DB sync: deya→org terminology cleanup (2026-05-21)
...
Aligned with the reference application (deya_context → organization_context):
...
- Indexes updated to match the reference application naming
...
- `user_activity_log`: standalone time index, renamed indexes to match the reference application
...
- `48d93f6` fix(migrations): add missing columns to loginlockouts to match the reference application
...
Thorough comparison between framework migrations and the live reference production database (Docker). Rule: "do it like the reference application" for all decisions.
...
## 🏁 Session 83 — Migration API helpers + the reference application characterization test fixes (2026-05-16)
...
**the reference application characterization tests** (commit `4c57288`):
...
### Commits
- `106182f` feat(database): add migration-support helpers for SchemaImport migrations
...
- `4c57288` test(characterization): fix the reference application tearDown cascade and add missing stubs
...
- **Fix B:** `SchemaMigrationsCharacterizationTest`: `connect(true)` → `connect(false)` με try-catch ώστε failed connection = skip (όχι RuntimeException)
...
## 🏁 Session 81 — SchemaImport Test Coverage: QueueManager, Worker, Health Checks (2026-05-15)
...
## 🏁 Session 80 — SchemaImport Test Coverage: ProcessQueue, DaemonOrchestrator, AbstractTask (2026-05-15)

### ✅ Ολοκληρώθηκε

**Testable Subclass pattern από the reference application εφαρμόστηκε στο framework** για όλα τα brought in Console/Queue features:
...
### Commits
- `dc52c3c` test(console/queue): add schema import tests for ProcessQueue, DaemonOrchestrator, AbstractTask
...
## 🏁 Session 77 — the reference application Schema SchemaImport: Complete Migration Audit & Implementation (2026-05-14)

### ✅ Ολοκληρώθηκε

**Comprehensive Schema Audit:**
- Extracted full the reference application database schema (143K lines) and identified ALL missing elements
- Created MIGRATION_AUDIT.md: complete comparison between PramnosFramework (43 migrations) and the reference application
...
- SchemaMigrationsCharacterizationTest.php: 550+ lines
...
- docs/1.2-new-features.md: new section "the reference application Schema SchemaImport — Phase 2"
...
### Commits
- `cb9df08` docs: add comprehensive migration audit comparing PramnosFramework vs the reference application
- `fac9f0a` feat(migrations): add missing application_settings, application_stats, and user_app_authorizations tables
- `c161469` test(characterization): add tests for the reference application schema import migrations
- `9b9700d` docs: update audit and feature docs with complete the reference application schema schema import

### 📊 Status
- **Completed:** 4 migrations (3 tables + 1 FK schema import migration)
...
### 🔍 Key Findings
- the reference application schema is significantly more advanced than initially identified
...
- Foreign key schema importing ensures referential integrity across all domains
...
### Known Status
- the reference application integration suite (5,176 tests) running but slow; verification pending
...
2. **Controller Default Pollution (the reference dev database)** — Watersupply::updateSupply() used corrupted cached groupid as default value when field not in PUT request
...
**Test Results:**
- ✅ All 5,176 the reference application integration tests pass
...
## 🏁 Session 69 — the reference application SchemaImports: OAuthPolicyHelper, Scopes, Helpers, Scaffolding Views (2026-05-12)

### Ολοκληρώθηκε
- **`OAuthPolicyHelper`** (`src/Pramnos/Auth/OAuthPolicyHelper.php`) — added `getAuthenticationMethods()`, `getGrantTypes()`, `getWebhookTypes()` descriptive registries brought in from the reference application `PermissionHelper`
...
### Commits
- `18a3dd9` — feat(auth): schema import OAuthPolicyHelper registries, Scopes.addDefaultScopesToToken, Helpers validators; add 51 scaffold views
...
- [x] **SchemaImport UW-461 regression fix** (c7230fe): JWT `client_credentials` grant (RFC 7523 `private_key_jwt`) now reuses the existing `applications.systemuser` instead of creating a new `sys_*` user on every token request. Fix: SELECT `systemuser` from applications AFTER JWT validation, INSERT new User only when NULL.
...
- [x] **`000028` oauth2_client_auth_methods**: Moved from `authserver` → `applications` schema (matches reference production). Added `is_primary` column. FK to `applications.appid`.
- [x] **`000029` oauth2_webhooks**: Complete rewrite — correct `applications` schema with the reference application-aligned columns (`webhook_id`, `endpoint_url`, `webhook_type`, `secret_key`, `retry_count`, `timeout_seconds`). Added `applications.create_webhook_event()` PL/pgSQL function (PostgreSQL only) for event fan-out.
...
- [x] **Deleted `auth/000027_add_gdpr_columns_to_users`**: the reference application uses dedicated GDPR tables (000021-000025 in `authserver` schema); GDPR columns on the `users` table were redundant and not used.
...
- [x] **`authserver/000031` replaced**: `CreateAuthserverUserDeyasTable` → `CreateAuthserverUserOrganizationsTable`. Both the table name (`authserver_organization_table`, default: `user_organizations`) and org column (`authserver_organization_column`, default: `organization_id`) are configurable via Settings so the reference application can override to `user_deyas`/`deyaid` in its `settings.php`.
...
- [x] **GDPR tables (000021-000025) verified**: All 5 tables (`user_activity_log`, `user_privacy_settings`, `user_consents`, `data_processing_records`, `gdpr_requests`) properly brought in in the `authserver` schema with TimescaleDB hypertable support. All have integration tests in MySQL, PostgreSQL and TimescaleDB suites.
...
- [x] **Migrations 000017–000026**: All 10 auth migrations now create tables in the `authserver` schema (PostgreSQL) / `authserver_` prefix (MySQL), matching the reference application's layout exactly:
...
- [x] **`users.locationid` removed** — the reference application-specific FK to locations table stripped from framework users migration
...
- [x] **App CLI scaffold (the reference application pattern)** — generates `{cliName}.php` (PHP entry point), `{cliName}` (bash wrapper: `docker-compose exec app php {cliName}.php "$@"`), and `src/Console.php` extending `\Pramnos\Console\Application`
...
### Phase 2: Queue System schema import (2026-05-05, session 33)
...
- [x] **`Pramnos\Queue\QueueItem`** — ORM model for `queueitems` table; configurable `getItemShowUrl/EditUrl/DeleteUrl()` hooks replace hardcoded the reference application URLs
- [x] **`Pramnos\Queue\QueueManager`** — full queue lifecycle: `addTask()`, `getNextTask()` (split pending/stalled queries), all `markTask*` transitions, `getStats()`, `purgeOldTasks()`; `getTasksDirectory()` / `getTasksNamespace()` hooks replace hardcoded the reference application namespace scan; `getQueueTableName()` hook; `createQueueItemModel()` factory
...
### Phase 2: DaemonOrchestrator schema import (2026-05-05, session 33)

- [x] **`Pramnos\Console\DaemonOrchestrator`** (`src/Pramnos/Console/DaemonOrchestrator.php`) — abstract process supervisor brought in from the reference application:
...
### Phase 2: CLI UX — CommandBase schema import (2026-05-05, session 32)

- [x] **`Pramnos\Console\CommandBase`** (`src/Pramnos/Console/CommandBase.php`) — schema import of `App\ConsoleCommands\CommandBase`:
  ...
  - Progress bar: `buildProgressBar(current, total, width=50)` — block-char `█` / `.` style extracted from the reference application commands
...
- **Note:** `PramnosStyle` commit (`bcbf4e9`) was reverted — wrong approach (invention vs schema import)
...
Bug fixes required after verifying against the reference application PostgreSQL test suite (5 176 tests):
...
- [x] **New constants** — `TIMESCALEDB` (alias for `FEATURE_TIMESCALEDB`), `JSONB` (alias for `FEATURE_JSONB`), `MATERIALIZED_VIEWS`, `ENUMS` — aligns with SchemaImport Spec Section 14.1.
...
- [x] `ifCapable(capability, callback, fallback)` — capability-conditional DDL per SchemaImport Spec Section 14.3
...
- **the reference application Integration Suite:** 5208/5208 tests passing (0 failures, 0 errors, 6 risky = pre-existing OAuth2 unexpected output) — runs against live PostgreSQL + TimescaleDB via Docker.
