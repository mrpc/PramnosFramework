...
PHP;
        ...
        /*
         * ── No account views are written, and that is the fix for two of them ──
         *
         * `src/Views/account/dashboard.html.php` and `profile.html.php` were written
         * here and **never rendered once**. `Account::display()` calls
         * `getView('dashboard')` and `profile()` calls `getView('profile')`, which
         * resolve to `src/Views/dashboard/` and `src/Views/profile/` — so a directory
         * called `account` is one nothing ever looks in.
         *
         * Worse than dead code, because they look like the account screens: somebody
         * wanting to change the account page finds them, edits them, reloads, and nothing
         * happens. They were also raw Tailwind where every other view is daisyUI, so the
         * difference was not even visible as a hint that they are not the live ones.
         *
         * And they are not moved to the right directory either, which is the tempting
         * fix. `scaffolding/themes/*\/views/dashboard/` and `.../profile/` already ship
         * working versions in the project's own theme; a copy under `src/Views/` would
         * take precedence over them and replace a maintained screen with a frozen one.
         * `project:publish-views` is how a project takes ownership of a view it wants to
         * change, and it copies the version that actually renders.
         */
    }
