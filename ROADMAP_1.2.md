...
- [ ] **Auto-run Mechanism (SchemaImport από the reference application):** Κατά το bootstrap της εφαρμογής, ο migration runner εκτελείται αυτόματα:
...
- [~] **Characterization Tests — `Model`:** *(μερικώς — PostgreSQL μόνο μέσω the reference application suite)* Κάλυψη `get()`, `save()`, `delete()`, column introspection, change tracking, και caching integration. Απαιτείται: επέκταση σε MySQL + TimescaleDB, επίσημα framework tests — **× 3 databases**.
- [~] **Characterization Tests — `DataTable`:** *(μερικώς — PostgreSQL μόνο μέσω the reference application suite)* Κάλυψη dynamic filtering, multi-column sorting, pagination, και παραγόμενο SQL output. Απαιτείται: επέκταση σε MySQL + TimescaleDB — **× 3 databases**.
...
- [~] **Characterization Tests — `Auth`:** *(μερικώς — PostgreSQL μόνο μέσω the reference application suite)* Κάλυψη credential lookup, session persistence, permission resolution. Απαιτείται: JWT issuance, login/logout flows, MySQL + TimescaleDB — **× 3 databases**.
- [~] **Characterization Tests — `User`:** *(μερικώς — PostgreSQL μόνο μέσω the reference application suite)* Κάλυψη create, update, lookup, role assignment. Απαιτείται: επέκταση σε MySQL + TimescaleDB — **× 3 databases**.
- [~] **Characterization Tests — `Logs`:** *(μερικώς — PostgreSQL μόνο μέσω the reference application suite)* Κάλυψη log insertion και query. Απαιτείται: επέκταση σε MySQL + TimescaleDB, ξεχωριστές assertions για hypertable path — **× 3 databases**.
...
- [ ] **`Api\\Apikey::getList()` result iteration contract:** Η μέθοδος κάνει `foreach ($result as $app)` και προσπελαύνει `$app->fields`, αλλά σε legacy paths το iterated item δεν εγγυάται object με `fields`, προκαλώντας warnings/σπασμένο hydration. Να ενοποιηθεί στο `while ($result->fetchNext())` pattern.
