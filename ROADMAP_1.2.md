...
- SchemaImport advanced application/authserver schema:
...
- Συγχρονισμός indexes, triggers, comments, policies με the reference application
...
- [ ] **`_getJsonList()` → delegate:** `_getJsonList()` καλεί εσωτερικά `_getApiList(format: 'datatables')` και αποδίδει το ίδιο αποτέλεσμα. Κανένα υπάρχον `getJsonList()` override στο the reference application δεν σπάει — BC αποδεδειγμένα διατηρείται.
