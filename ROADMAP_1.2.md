...
- [x] **`DatabaseCapabilities` — Runtime Detection & Graceful Fallback:**
  ...
  - [x] SchemaImport Spec Section 14.1 alignment complete (constants, static cache, new capabilities)
  ...
  - Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 14
...
## 📦 Φάση 2: the reference application Features Port
*Μεταφορά και ενσωμάτωση ώριμων υποσυστημάτων από το the reference application project στο Core Framework. Κάθε feature ενσωματώνεται ως αυτόνομο, opt-in component — ενεργοποιείται μέσω του Feature Registry (Φάση 4) και φέρει τα δικά του system migrations.*

> ⚠️ **Προϋπόθεση:** Η Φάση 2 εξαρτάται πλήρως από τη Φάση 4. Τα schema import features χρησιμοποιούν Feature Registry για ενεργοποίηση, Service Providers για bootstrap, και System Migrations για schema. **Καμία από τις παρακάτω υλοποιήσεις δεν μπορεί να ολοκληρωθεί πριν τελειώσει η Φάση 4.** Σωστή σειρά υλοποίησης: Φάση 1 → Φάση 4 → Φάση 2.

- **Queues System** *(feature key: `queue`)*: SchemaImport από the reference application — configurable hooks για URLs, task namespace, table name, dashboard title. Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 5.
...
- **Token Action Tracking** *(feature key: `auth`)*: Πλήρης αναβάθμιση του `tokenactions` + `urls` schema — hypertable support, execution time tracking, MySQL compat. Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 9.
...
- **OAuth Server** *(feature key: `authserver`)*: Ενσωμάτωση του πλήρους OAuth2 server (league/oauth2-server). Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 3.
...
- **Authentication System** *(feature key: `auth`)*: Αναβαθμισμένο σύστημα auth — login lockout, 2FA (TOTP), GDPR, RBAC. Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 4.
...
- **Messaging** *(feature key: `messaging`)*: Σύστημα μηνυμάτων — private messages, notifications, mass broadcast, email queue. Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 7.
...
- [x] **CLI UX Improvements:** SchemaImport `CommandBase` από the reference application — lock-file job guards, terminal control (cursor/clear/size), bordered dashboard rendering, block-character progress bar, text utils (formatBytes, formatTime, visibleLength, wrapDashboardText). the reference application commands μπορούν να extend-άρουν `Pramnos\Console\CommandBase`.
- [x] **Daemon Orchestrator:** SchemaImport `DaemonOrchestrator` abstract class από the reference application — generic reconcile engine, desired-vs-actual process management, stop-file mechanism, flock singleton guard, dedup scan, git-hash restart detection, interactive dashboard. Apps override abstract methods (`buildDesiredProcesses`, `getDashboardTitle`, `getEntryPoint`) και hooks (`isOrchestratorEnabled`). the reference application's orchestrator μπορεί να κάνει extend `Pramnos\Console\DaemonOrchestrator` και να παρέχει μόνο την app-specific λογική.
...
[Φάση 1] Grammar/Adapter Pattern
         ...
         ↓
[Φάση 1] DatabaseCapabilities alignment με SchemaImport Spec
...
[Φάση 5] Επίσημα Characterization Tests × 3 databases
         ...
         ⚠️ Τα υπάρχοντα the reference application tests μετρούν ως μερική
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ΣΤΑΔΙΟ Ε: SchemaImport Features (Φάση 2) — μετά τη Φάση 4
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
> **Παρατήρηση για την τρέχουσα κατάσταση:** Η Φάση 1 Internal Migration ολοκληρώθηκε (Model, DataTable) **χωρίς** προηγούμενα επίσημα characterization tests × 3 databases. Τα the reference application tests χρησιμοποιούν ως de facto characterization suite αλλά καλύπτουν μόνο PostgreSQL + TimescaleDB. Προτού αγγιχτεί οποιοδήποτε άλλο class (Auth, User, Logs, Adjacencylist), τα επίσημα tests πρέπει να γραφούν.


### 🆕 Backlog: the reference application Schema SchemaImport & Open Jira Issues
...
#### the reference application Schema SchemaImport
...
- [ ] Συγχρονισμός indexes, comments, default values με the reference application schema
...
*Web-accessible admin panel ενσωματωμένο στο framework. Εμπνευσμένο από το dashboard του the reference application (`src/Controllers/Home.php` + `src/Controllers/Users.php`). Μετά την υλοποίηση το the reference application θα αντικαθιστήσει τον κώδικα Home/Users με thin wrappers (βλ. the reference application-Cleanup-Guide.md).*
...
- [ ] **Token detail page:** paginated action history (endpoint, method, execution_time_ms, timestamp) — compatible με `tokenactions` table από Auth schema import.
...
#### Git Info Widget
*Παρόμοιο με το footer git modal του the reference application. Διαβάζει `.git/HEAD` / `.git/objects/` απευθείας — zero `exec()`.*
...
#### Πρόβλημα σήμερα
Ένα project σαν το the reference application έχει:
- `www/index.php` → `new Application()` → `app/app.php` → controllers σε `the reference application\Controllers\`
- `www/api/index.php` → `new Api('api')` → `app/api.php` → controllers σε `the reference application\Api\Controllers\`
...
$router->group([
    ...
    'namespace'  => 'the reference application\\Api\\Controllers',
], function ($r) {
...
$router->group([
    ...
    'namespace'  => 'the reference application\\Controllers',
], function ($r) {
    $r->get('/users',       'Users@index');
});
```
...
> **the reference application migration:** βλ. `the reference application-Cleanup-Guide.md` Phase 7 (προστίθεται όταν αρχίσει η υλοποίηση).
...
- [x] **`_getJsonList()` → delegate:** `_getJsonList()` καλεί εσωτερικά `_getApiList()` και αποδίδει DT 1.9 envelope για BC. Κανένα υπάρχον `getJsonList()` override στο the reference application δεν σπάει.
...
- [ ] **the reference application migration:** βλ. `the reference application-Cleanup-Guide.md` Phase 8 — custom `getJsonList()` overrides μεταφέρονται σταδιακά.

> **BC:** `_getJsonList()` και όλα τα custom `getJsonList()` overrides στο the reference application συνεχίζουν να λειτουργούν αναλλοίωτα. Η μετάβαση γίνεται model-by-model, view-by-view.
...
- [ ] **the reference application migration:** αντικαθιστήσει `src/Api/apidoc.json` + `scripts/apidoc-to-openapi.js` με τις νέες εκδόσεις — output identικό, χωρίς αλλαγές στα controllers
...
*Στο the reference application υπάρχουν δύο bare PHP scripts (`githook.php`, `githook-dev.php`) που εκτελούν `git pull` όταν το GitHub/Bitbucket στέλνει push event. Έχουν κρίσιμο security gap: **δεν επαληθεύουν HMAC signature** — ο server εκτελεί `shell_exec` σε οποιοδήποτε στέλνει POST request. Αυτή η φάση φτιάχνει ασφαλή, configurable webhook infrastructure ως framework component.*

#### Πρόβλημα σήμερα (the reference application)

```php
// githook.php — κανένας έλεγχος πηγής:
$payload = json_decode(file_get_contents('php://input'));
// ... branch detection ...
$content = shell_exec('cd /home/the reference application/public_html && git pull origin master');
```

- Δεν υπάρχει επαλήθευση `X-Hub-Signature-256` (GitHub HMAC-SHA256)
- Hardcoded paths (`/home/the reference application/public_html/`)
...
## the reference application Schema SchemaImport Tasks
...
*SchemaImport και βελτίωση όλων των admin management controllers που υπάρχουν στο the reference application — ώστε κάθε νέα εφαρμογή να τα κληρονομεί out-of-the-box μέσω wrappers.*
...
- [ ] **`PermissionsController`** στο `\Pramnos\Auth\Controllers\` (όταν feature `authserver` + αν `authserver.permissions` schema υπάρχει — Φάση the reference application Schema SchemaImport):
...
- [ ] **`OrganizationsController`** στο `\Pramnos\Application\Controllers\` (πάντα — εφόσον ολοκληρωθεί το `public.organizations` migration από Φάση the reference application Schema SchemaImport):
...
*SchemaImport της λογικής `getActiveUsers()` / `getPerformanceSummary()` από το the reference application Home controller σε επαναχρησιμοποιήσιμους framework services.*
