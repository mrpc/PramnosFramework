...
- [x] **Queues System** *(feature key: `queue`)*: SchemaImport `QueueManager`, `Worker`, `AbstractTask`, `TaskInterface`, `QueueItem`, `ProcessQueue`, `CleanupQueue` από the reference application. Configurable hooks για URLs, task namespace, table name, dashboard title. the reference application μπορεί να κάνει subclass και να override μόνο τα app-specific μέρη.
