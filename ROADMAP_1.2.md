...
- **OAuth Server** *(feature key: `authserver`)*: Ενσωμάτωση του πλήρους OAuth2 server (league/oauth2-server). Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 3.
  ...
  - [x] Auth Controllers (partial): `Discovery.php` (OIDC discovery + JWKS + RFC 8414 + health), `Session.php` (check/heartbeat/info/refresh, Bearer+session dual-auth), `TwoFactorAuth.php` (wraps TwoFactorAuthService), `Gdpr.php` (GDPR requests + deauthorize + notifyChange via WebhookService::queueEvent()) — 24 unit tests; `Oauth.php`, `Device.php`, `Dashboard.php` deferred to next session
