...
- [x] **`DatabaseCapabilities` — Runtime Detection & Graceful Fallback:**
  ...
  - Αναλυτική προδιαγραφή: βλ. `the reference application-Backport-Features.md` Section 14
...
- **Queues System** *(feature key: `queue`)*: Backport από the reference application — configurable hooks για URLs, task namespace, table name, dashboard title. Αναλυτική προδιαγραφή: βλ. `the reference application-Backport-Features.md` Section 5.
...
- **Token Action Tracking** *(feature key: `auth`)*: Πλήρης αναβάθμιση του `tokenactions` + `urls` schema — hypertable support, execution time tracking, MySQL compat. Αναλυτική προδιαγραφή: βλ. `the reference application-Backport-Features.md` Section 9.
...
- **OAuth Server** *(feature key: `authserver`)*: Ενσωμάτωση του πλήρους OAuth2 server (league/oauth2-server). Αναλυτική προδιαγραφή: βλ. `the reference application-Backport-Features.md` Section 3.
...
- **Authentication System** *(feature key: `auth`)*: Αναβαθμισμένο σύστημα auth — login lockout, 2FA (TOTP), GDPR, RBAC. Αναλυτική προδιαγραφή: βλ. `the reference application-Backport-Features.md` Section 4.
...
- **Messaging** *(feature key: `messaging`)*: Σύστημα μηνυμάτων — private messages, notifications, mass broadcast, email queue. Αναλυτική προδιαγραφή: βλ. `the reference application-Backport-Features.md` Section 7.
...
- [x] **Auto-run Mechanism (Backport από the reference application):** Κατά το bootstrap της εφαρμογής, ο migration runner εκτελείται αυτόματα:
...
### 🆕 Backlog: the reference application Schema Backport & Open Jira Issues
...
#### the reference application Schema Backport
...
## the reference application Schema Backport Tasks
...
*Backport και βελτίωση όλων των admin management controllers που υπάρχουν στο the reference application — ώστε κάθε νέα εφαρμογή να τα κληρονομεί out-of-the-box μέσω wrappers.*
...
- [ ] **`PermissionsController`** στο `\Pramnos\Auth\Controllers\` (όταν feature `authserver` + αν `authserver.permissions` schema υπάρχει — Φάση the reference application Schema Backport):
...
- [ ] **`OrganizationsController`** στο `\Pramnos\Application\Controllers\` (πάντα — εφόσον ολοκληρωθεί το `public.organizations` migration από Φάση the reference application Schema Backport):
...
*Backport της λογικής `getActiveUsers()` / `getPerformanceSummary()` από το the reference application Home controller σε επαναχρησιμοποιήσιμους framework services.*
