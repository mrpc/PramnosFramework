...
- **Authentication System** *(feature key: `auth`)*: Αναβαθμισμένο σύστημα auth — login lockout, 2FA (TOTP), GDPR, RBAC. Αναλυτική προδιαγραφή: βλ. `the reference application-SchemaImport-Features.md` Section 4.
  ...
  - [ ] 2FA view templates (setup, challenge, backup codes) — `TwoFactorAuth.php` controller (display/setup/disable/backup/status) είναι έτοιμος (βλ. Auth Controllers παραπάνω)· λείπουν τα HTML/PHP view templates για framework-level default views ή scaffolding
  - [ ] GDPR user-facing views (export, delete, consents) — `Gdpr.php` υλοποιεί API endpoints (request/status/listRequests/deauthorizeAll/notifyChange)· λείπουν τα user-facing HTML pages για αυτοεξυπηρέτηση GDPR
