...
<div class="px-4 py-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="card bg-blue-600 text-white">
            <div class="p-5">
                <div class="text-white/75 text-xs">Active Users (now)</div>
                <div class="text-3xl font-bold"><?php echo (int)($this->activeUsers['now'] ?? 0); ?></div>
                <div class="text-white/75 text-xs">Last 24h: <?php echo (int)($this->activeUsers['last_24h'] ?? 0); ?></div>
            </div>
        </div>
        <div class="card bg-green-600 text-white">
            <div class="p-5">
                <div class="text-white/75 text-xs">API Requests (24h)</div>
                <div class="text-3xl font-bold"><?php echo (int)($this->apiStats['total_requests'] ?? 0); ?></div>
                <div class="text-white/75 text-xs">Errors: <?php echo number_format(($this->apiStats['error_rate'] ?? 0) * 100, 1); ?>%</div>
            </div>
        </div>
        <div class="card bg-sky-600 text-white">
            <div class="p-5">
                <div class="text-white/75 text-xs">Avg Latency (24h)</div>
                <div class="text-3xl font-bold"><?php echo number_format($this->apiStats['avg_execution_time'] ?? 0, 0); ?> ms</div>
                <div class="text-white/75 text-xs">p95: <?php echo number_format($this->apiStats['p95_execution_time'] ?? 0, 0); ?> ms</div>
            </div>
        </div>
        <div class="card bg-slate-600 text-white">
            <div class="p-5">
                <div class="text-white/75 text-xs">DB Size</div>
                ...
                <div class="text-white/75 text-xs">Connections: <?php echo (int)($this->dbStats['connections_active'] ?? 0); ?></div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-xs border border-gray-200 mb-6">
        <div class="px-5 py-3 bg-gray-50 border-b border-gray-200 font-semibold text-sm flex justify-between items-center">
            <span>Database</span>
            <a href="<?php echo adminUrl('dashboard/database'); ?>" class="text-blue-600 text-xs font-normal hover:underline">View Details &rarr;</a>
        </div>
        <div class="p-5">
    ...
    <div class="bg-white rounded-xl shadow-xs border border-gray-200 mb-6">
        <div class="px-5 py-3 bg-gray-50 border-b border-gray-200 font-semibold text-sm flex justify-between items-center">
            <span>Cache</span>
            <a href="<?php echo adminUrl('dashboard/cache'); ?>" class="text-blue-600 text-xs font-normal hover:underline">View Details &rarr;</a>
        </div>
