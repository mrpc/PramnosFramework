        ...
        <div class="p-5">
            <form method="post" action="<?php echo sURL; ?>settings/save">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="key" class="w-full px-3 py-2 border border-gray-300 rounded text-sm" required
                        value="<?php echo htmlspecialchars($this->key ?? ''); ?>"
                        <?php echo !$this->isNew ? 'readonly' : ''; ?>>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Value</label>
                    <textarea name="value" class="w-full px-3 py-2 border border-gray-300 rounded text-sm" rows="4"><?php echo htmlspecialchars($this->value ?? ''); ?></textarea>
                </div>
                <div class="flex gap-2">
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700">Save</button>
                    <a href="<?php echo sURL; ?>settings/list" class="px-4 py-2 border border-gray-300 text-gray-700 text-sm rounded hover:bg-gray-50">Cancel</a>
                </div>
            </form>
        </div>
    </div>
